...
function carregaContatos() {
	...
	if (tamanho > 0) {
		...
		'<img src="img/config.svg" class="icon_config" onclick="ativaConfig()" id="icon-config"></div>' +
		...
		'<label class="importar" for="arquivo">' +
		...
		'<div class="src"><input placeholder="Buscar contato" type="text" name="busca" id="busca" onkeydown="inputbusca(event)">' +
		...
		for (let i = tamanho; i >= 1; i--) {
			...
			history.innerHTML += '<div class="contato" id="'+ i +'">' +
...
function excluiContato(num) {
	let ctt = document.getElementById(num)
	...
	ctt.style.cssText = 'animation: delete 1s ease-in forwards;'
	setTimeout(() => {  
...
function ativaConfig() {
	...
	let icon = document.getElementById('icon-config')
	config.classList.toggle('show_config')
	icon.classList.toggle('icon_config_click')
}

function showLimpaContatos() {
	let modal = document.getElementById('modal-limpa-contatos')
	let bkModal = document.getElementById('background-modal')
	bkModal.classList.toggle('show_background_modal')
...
function buscarContato() {
	...
	if (res != null && res != '' && res != false && res != -1 && res != '-1') {
		...
		resultado.innerHTML = '<div class="contato" id="'+ res +'" style="width: 98.9% !important; margin-top: 5px">' +
